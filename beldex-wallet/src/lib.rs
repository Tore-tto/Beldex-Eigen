...
use beldex::consensus::encode::VarInt;
use beldex::cryptonote::hash::Hashable;
use beldex_rpc::beldexd;
use beldex_rpc::beldexd::{BeldexdRpc as _, GetBlockResponse};
use rand::Rng;
...
mod tests {
    use super::*;
    use beldex_harness::image::Beldexd;
    use beldex_rpc::beldexd::{Client, GetOutputsOut};
    use testcontainers::clients::Cli;

    #[tokio::test]
    async fn get_outs_for_key_offsets() {
        let cli = Cli::default();
        let container = cli.run(Beldexd);
