...
impl Wallet {
        ...
        match client.open_wallet(name.clone()).await {
            Err(error) => {
                ...
                client.create_wallet(name.clone(), "English".to_owned()).await.context(
                    "Unable to create Beldex wallet, please ensure that the beldex-wallet-rpc is available",
                )?;

                tracing::debug!(beldex_wallet_name = %name, "Created Beldex wallet");
            }
            Ok(_) => tracing::debug!(beldex_wallet_name = %name, "Opened Beldex wallet"),
        }
    ...
    pub async fn connect(client: wallet::Client, name: String, env_config: Config) -> Result<Self> {
        let main_address =
            beldex::Address::from_str(client.get_address(0).await?.address.as_str())?;

        Ok(Self {
            inner: Mutex::new(client),
            network: env_config.beldex_network,
            name,
            main_address,
            sync_interval: env_config.beldex_sync_interval(),
        })
    }
                ...
                .await
            {
                Ok(sweep_all) => {
                    for tx in sweep_all.tx_hash_list {
                        tracing::info!(
                            %tx,
                            beldex_address = %self.main_address,
                            "Beldex transferred back to default wallet");
                    }
                }
                Err(error) => {
                    tracing::warn!(
                        address = %self.main_address,
                        "Failed to transfer Beldex to default wallet: {:#}", error
                    );
                }
            },
            Err(error) => {
    ...
    pub async fn transfer(&self, request: TransferRequest) -> Result<TransferProof> {
        ...
        let res = inner
            .transfer_single(0, amount.as_atomic(), &destination_address.to_string())
            .await?;
        ...
        tracing::debug!(
            ...
            to = %public_spend_key,
            tx_id = %res.tx_hash,
            "Successfully initiated Beldex transfer"
        );
    ...
    pub async fn watch_for_transfer(&self, request: WatchRequest) -> Result<(), InsufficientFunds> {
        ...
        tracing::info!(
            ...
            "Waiting for Beldex transaction finality"
        );
    ...
    pub async fn refresh(&self, max_attempts: usize) -> Result<Refreshed> {
        ...
        for i in 1..=max_attempts {
            tracing::info!(name = %self.name, attempt=i, "Syncing Beldex wallet");

            let result = self.inner.lock().await.refresh().await;

            match result {
                Ok(refreshed) => {
                    tracing::info!(name = %self.name, "Beldex wallet synced");
                    return Ok(refreshed);
                }
                Err(error) => {
                    ...
                    let height = match self.inner.lock().await.get_height().await {
                        ...
                        Err(_) => {
                            tracing::warn!(name = %self.name, "Failed to fetch Beldex wallet height during sync");
                            "unknown".to_string()
                        }
                    };

                    tracing::warn!(attempt=i, %height, %attempts_left, name = %self.name, %error, "Failed to sync Beldex wallet");
...
async fn wait_for_confirmations<C: beldex_rpc::wallet::BeldexWalletRpc<reqwest::Client> + Sync>(
    client: &Mutex<C>,
    ...
    while seen_confirmations < conf_target {
        ...
        let received = Amount::from_atomic(tx.received);
        ...
        if tx.confirmations > seen_confirmations {
            ...
            tracing::info!(
                ...
                "Received new confirmation for Beldex lock tx"
            );
        }
    }
...
mod tests {
    ...
    async fn visual_log_check() {
        ...
        assert_eq!(
            writer.captured(),
            r" INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=1 needed_confirmations=5
 INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=3 needed_confirmations=5
 INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=5 needed_confirmations=5
"
        );
    }
    ...
    async fn reopens_wallet_in_case_not_available() {
        ...
        assert_eq!(
            writer.captured(),
            r" INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=1 needed_confirmations=5
DEBUG swap::beldex::wallet: Opening wallet `foo-wallet` because no wallet is loaded
 INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=3 needed_confirmations=5
 INFO swap::beldex::wallet: Received new confirmation for Beldex lock tx txid=<FOO> seen_confirmations=5 needed_confirmations=5
"
        );
    ...
    impl beldex_rpc::wallet::BeldexWalletRpc<reqwest::Client> for DummyClient {
        async fn open_wallet(
            &self,
            _: String,
        ) -> Result<wallet::WalletOpened, beldex_rpc::jsonrpc::Error<reqwest::Error>> {
        ...
        async fn check_tx_key(
            ...
            _: String,
            _: String,
        ) -> Result<wallet::CheckTxKey, beldex_rpc::jsonrpc::Error<reqwest::Error>> {
            let index = self.check_tx_key_invocations.fetch_add(1, Ordering::SeqCst);

            self.check_tx_key_responses[index as usize]
                .clone()
                .map_err(|(code, message)| {
                    beldex_rpc::jsonrpc::Error::JsonRpc(beldex_rpc::jsonrpc::JsonRpcError {
                        code,
                        message,
                        data: None,
                    })
        ...
        async fn send_request<P>(
            &self,
            _: String,
        ) -> Result<beldex_rpc::jsonrpc::Response<P>, reqwest::Error>
        where
