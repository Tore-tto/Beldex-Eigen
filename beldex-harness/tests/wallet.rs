use beldex_harness::{Beldex, BeldexWalletRpc};
use beldex_rpc::wallet::BeldexWalletRpc as _;
...
async fn fund_transfer_and_check_tx_key() {
    ...
    let tc = Cli::default();
    let (beldex, _beldexd_container, _wallet_containers) =
        Beldex::new(&tc, vec!["alice", "bob"]).await.unwrap();
    let alice_wallet = beldex.wallet("alice").unwrap();
    let bob_wallet = beldex.wallet("bob").unwrap();

    // hbeldex.init_and_start_miner().await.unwrap();
    beldex.init_wallet("alice", vec![fund_alice]).await.unwrap();
    beldex.init_wallet("bob", vec![fund_bob]).await.unwrap();
...
async fn wait_for_wallet_to_catch_up(wallet: &BeldexWalletRpc, expected_balance: u64) {
    let max_retry = 15;
    let mut retry = 0;
    loop {
        retry += 1;
